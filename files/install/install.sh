...
echo 'deb http://us.archive.ubuntu.com/ubuntu/ xenial main universe restricted' > /etc/apt/sources.list
echo 'deb http://us.archive.ubuntu.com/ubuntu/  xenial-updates main universe restricted' >> /etc/apt/sources.list
...
# Install misc
apt-get install -qy --force-yes --no-install-recommends nano \
							lxterminal
